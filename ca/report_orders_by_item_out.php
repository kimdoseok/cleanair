	...
	if ($method == "html") {
		if ($show_format == "woa" || $show_format == "wa") {
			if ($show_detail== "t") {
				...
				if ($ua_arr["userauth_allow"]=="t") include("report_orders_by_item_detail_html.php");
				else include("permission.php");
				
			} else {
				$ua_arr = $ua->getUserAuthsTwoCode($_SERVER["PHP_AUTH_USER"], "rpt_sale_item_s");
				if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
				if ($ua_arr["userauth_allow"]=="t") include("report_orders_by_item_summary_html.php");
				else include("permission.php");
			...
			if ($ua_arr["userauth_allow"]=="t") {
				include("report_orders_by_item_rr_html.php");
			} else {
		...
		if ($show_detail== "t") {
			$ua_arr = $ua->getUserAuthsTwoCode($_SERVER["PHP_AUTH_USER"], "rpt_sale_item_d");
			if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
			if ($ua_arr["userauth_allow"]=="t") include("report_orders_by_item_detail_pdf.php");
			else include("permission.php");
			
		} else {
			$ua_arr = $ua->getUserAuthsTwoCode($_SERVER["PHP_AUTH_USER"], "rpt_sale_item_s");
			if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
			if ($ua_arr["userauth_allow"]=="t") include("report_orders_by_item_summary_pdf.php");
			else include("permission.php");
