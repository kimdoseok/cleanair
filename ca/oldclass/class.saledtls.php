...
//echo $this->query;
	...
	function getSaleDtlsRange($dfr="", $dto="", $ifr="", $ito="", $ty="s") { // s for summary, d for detail
		...
		if ($ty=="s") {
			...
			$groupby = " GROUP BY l.slsdtl_item_code ";
			$orderby = " ORDER BY l.slsdtl_item_code ";
			$from = " FROM slsdtls l, sales h ";
		} else {
			$select = " SELECT h.sale_id, h.sale_date, c.cust_code, c.cust_name, c.cust_city, c.cust_state, l.slsdtl_item_code, l.slsdtl_item_desc, l.slsdtl_qty, l.slsdtl_cost ";
