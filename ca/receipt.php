...
<?php
	...
	if ($ty == "a") {
		...
		if ($ua_arr["userauth_allow"]=="t") include("receipt_add.php");
      ...
      $_SESSION["olds"]=$olds;
		...
		if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
		if ($ua_arr["userauth_allow"]=="t") include("receipt_edit.php");
		else include("permission.php");
		...
		$numrows = $c->getReceiptRows();
		$d = new Datex();
		$oldrec = $c->getTextFields($dir, $rcpt_id);
		if ($oldrec) {
			foreach ($oldrec as $k => $v) $$k = $v;
			if (!empty($rcpt_id)) $cnt = 1;
		}
		...
		if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
		if ($ua_arr["userauth_allow"]=="t") include("receipt_view.php");
		else include("permission.php");
		
	} else  {
		$ua_arr = $ua->getUserAuthsTwoCode($_SERVER["PHP_AUTH_USER"], "rcpt_list");
		if ($_SERVER["PHP_AUTH_USER"]=="admin") $ua_arr["userauth_allow"]="t";
		if ($ua_arr["userauth_allow"]=="t") include("receipt_list.php");
		else include("permission.php");
