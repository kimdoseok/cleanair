...
function setCode(value, name, addr1, addr2, addr3, city, state, zip, taxrate, bal, wday, tel) {
	var f = document.forms[0];

	var s = self.opener.document.forms[0];
	self.opener.document.forms[0].<?= $objname ?>.value = value;
	self.opener.document.forms[0].purch_vend_name.value = name;
	self.opener.document.forms[0].purch_vend_addr1.value = addr1;
	self.opener.document.forms[0].purch_vend_addr2.value = addr2;
	
	self.opener.document.forms[0].purch_vend_addr3.value = addr3;
	
	self.opener.document.forms[0].purch_vend_city.value = city;
	
	self.opener.document.forms[0].purch_vend_state.value = state;
	
	self.opener.document.forms[0].purch_vend_zip.value = zip;
	self.opener.document.forms[0].purch_vend_tel.value = tel;
...
?>
                    <td width="10%" align="center"> 
						<a href="javascript:setCode('<?= addslashes($recs[$i]["vend_code"] ?? "") ?>','<?= addslashes($recs[$i]["vend_name"] ?? "") ?>','<?= addslashes($recs[$i]["vend_addr1"] ?? "") ?>','<?= addslashes($recs[$i]["vend_addr2"] ?? "") ?>','<?= addslashes($recs[$i]["vend_addr3"] ?? "") ?>','<?= addslashes($recs[$i]["vend_city"] ?? "") ?>','<?= addslashes($recs[$i]["vend_state"] ?? "") ?>','<?= addslashes($recs[$i]["vend_zip"] ?? "") ?>','<?= addslashes($xarr["taxrate_pct"] ?? "") ?>','<?= addslashes($recs[$i]["vend_balance"] ?? "") ?>','<?= addslashes($wday ?? "") ?>','<?= addslashes($recs[$i]["vend_tel"] ?? "") ?>','<?= addslashes($recs[$i]["vend_slsrep"] ?? "") ?>');"><?= $recs[$i]["vend_code"] ?? "" ?></a>
					</td>
                    <td width="30%"><?= $recs[$i]["vend_name"] ?? "" ?></td>
                    <td width="30%"><?= $recs[$i]["vend_addr1"] ?? "" ?></td>
                    <td width="15%"><?= $recs[$i]["vend_city"] ?? "" ?></td>
