  ...
  <tr> 
    <td><table width="100%" border="0" cellpadding="0" cellspacing="1" bgcolor="black">
        <tr align="center" bgcolor="silver"> 
          ...
          <td width="15%"><strong><font size="2" face="Arial, Helvetica, sans-serif">Date</font></strong></td>
          <td width="65%"><strong><font size="2" face="Arial, Helvetica, sans-serif">Description</font></strong></td>
...
?>
        <tr bgcolor="white"> 
          <td align="center">
            <font size="2" face="Arial, Helvetica, sans-serif"><?= $pick_arr[$j]["pick_id"] ?></font>
          </td>
          <td align="center"><font size="2" face="Arial, Helvetica, sans-serif"><?= $pick_arr[$j]["pick_date"] ?></td>
          <td>
            <font size="2" face="Arial, Helvetica, sans-serif">Invoice: <?= $pick_arr[$j]["pick_id"] ?>
<?php
	    ...
	    if ($pdtl_num>0) echo ")";
      if ($pick_arr[$j]["pick_freight_amt"]!=0) echo " &nbsp; &nbsp; Freight: ".number_format($pick_arr[$j]["pick_freight_amt"],2,",",".");
?>
	         </font>
          </td>
          <td align="right">
            <font size="2" face="Arial, Helvetica, sans-serif"><?= $pick_arr[$j]["pick_total"] ?></font>
          </td>
        </tr>
<?php
			...
			for ($k=0;$k<$rec_num;$k++) {
				if (!empty($recs[$k])) {
					...
					if ($recs[$k]["slsdtl_taxable"]=="t") {
						$tax_amt = $amt*$pick_arr[$j]["pick_taxrate"]/100;
					} else {
						$tax_amt = 0.00;
					}
?>
			...
			for ($k=0;$k<$rec_num;$k++) {
				if (!empty($recs[$k])) {
					...
					if ($recs[$k]["cmemodtl_taxable"]=="t") {
						$tax_amt = $amt*$pick_arr[$j][cmemodtl_taxrate]/100;
					} else {
						$tax_amt = 0.00;
					}
?>
...
?>
        <tr bgcolor="white"> 
          ...
          <td align="right"><font size="2" face="Arial, Helvetica, sans-serif"><?= number_format(($rcpt_arr[$j]["rcpt_amt"]+$rcpt_arr[$j]["rcpt_disc_amt"])*-1,2,".",",") ?></font></td>
        </tr>
<?php
		$recs = $rd->getRcptDtlsList($rcpt_arr[$j]["rcpt_id"]);
		if ($recs) {
?>
		<tr>
		  <td colspan="4" width="100%" bgcolor="white">
