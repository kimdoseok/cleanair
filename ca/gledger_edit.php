<?php
	...
	if ($_SESSION[$gledger_edit]) {
		$recs = $_SESSION[$gledger_edit];
		if ($recs[gldgr_id] != $gldgr_id) $recs = $c->getGLedger($gldgr_id);
		if (!empty($recs)) foreach($recs as $k => $v) $$k = $v; 
	} else {
		$recs = $c->getGLedger($gldgr_id);
		if (!empty($recs)) foreach($recs as $k => $v) $$k = $v; 
	}
	$_SESSION[$gledger_edit] = $recs;

	if ($_SESSION[$jrnltrxs_edit]) {
		$recs = $_SESSION[$jrnltrxs_edit];
		if ($recs[0]["jrnltrx_ref_id"] != $gldgr_id) $recs = $j->getJrnlTrxsList("g", $gldgr_id);
	} else {
		$recs = $j->getJrnlTrxsList("g", $gldgr_id);
	}
	$_SESSION[$jrnltrxs_edit] = $recs;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <form method=post action="gl_proc.php">
    <?= $f->fillHidden("cmd","") ?>
    <?= $f->fillHidden("ty","e") ?>
    <?= $f->fillHidden("gldgr_id", $gldgr_id) ?>
    <tr> 
      <td align="right"><strong> Edit General Ledger </strong></td>
    </tr>
    <tr> 
		  ...
		  <tr>
            ...
            <tr> 
              <td width="97" bgcolor="silver"> 
                ...
                &nbsp;</td>
              <td> 
                <?= $f->fillTextBoxRO("gldgr_user_code", $gldgr_user_code, 20, 32, "inbox") ?>
              </td>
            </tr>
            <tr> 
              <td width="97" bgcolor="silver"> 
                ...
                :</td>
              <td> 
                <?= $f->fillTextBoxRO("balance", sprintf("%0.2f", $balance) , 20, 32, "inbox") ?>
              </td>
            </tr>
          </table></td>
        </tr>
    ...
    <tr> <td align="left">
      <TABLE width="100%" bgcolor="silver" border="0" cellspacing="1" cellpadding="0">
        ...
        <Th bgcolor="gray" align="center" width="35%"><font color="white">Account 
          #</font></TD> 
        ...
        <Th bgcolor="gray" align="center" width="20%">&nbsp;</TD> </TR>
        <TR> 
          ...
          <TD bgcolor="white" align="center">
            <?= $f->fillHidden("jrnltrx_ref_id", $jrnltrx_ref_id) ?>
            <?= $f->fillTextBox("jrnltrx_acct_code", $jrnltrx_acct_code , 16, 16, "inbox") ?>
            <b><font size="2"><a href="javascript:openAcctBrowse('jrnltrx_acct_code')">Lookup</a></font></b></TD>
          <TD bgcolor="white" align="center">
            <?= $f->fillTextBox("jrnltrx_amt", $jrnltrx_amt+0 , 10, 32, "inbox") ?>
          </TD>
          <?php
	$debit = "";
	$credit = "";
...
?>
          <TD bgcolor="white" align="center"><INPUT TYPE="radio" NAME="jrnltrx_dc" value="d" <?= $debit ?>>
            Debit 
            <INPUT TYPE="radio" NAME="jrnltrx_dc" value="c" <?= $credit ?>>
            Credit</TD>
          <TD bgcolor="white" align="center"><INPUT TYPE="button" name="apply" value="Apply" onClick="applDtl()"> 
            <INPUT TYPE="button" name="clear" value="New" onClick="clearDtl()"> 
          </TD>
        </TR>
      </TABLE></td>
    </tr>
...
?>
          <td width="5%" align="center"> <a href="<?= htmlentities($_SERVER['PHP_SELF'])."?ty=e&did=$i&gldgr_id=$gldgr_id" ?>"> 
            <?= $i+1 ?>
            </a> </td>
          ...
          <td width="5%" align="center"> <a href="gl_proc.php?ty=e&cmd=gldgr_detail_sess_del&gldgr_id=<?= $gldgr_id ?>&did=<?= $i ?>"> 
            <?= $label[$lang]["Del"] ?>
            </a> </td>
          </tr>
          <?php
