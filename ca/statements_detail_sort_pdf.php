	...
	if ($zero_balance == "z" || ($zero_balance != "z" && $balance != 0)) {
		...
		for ($j = 0; $j < $out_num; $j++) {
			if ($out_arr[$j][0] == "I") {
				...
				if ($out_arr[$j][2]["pick_freight_amt"] != 0) {
					$tmp["fa"] = "Freight: " . number_format($out_arr[$j][2]["pick_freight_amt"], 2, ".", ",");
					//$tmp["fa"] = "Freight: ";
				}
				...
				if ($details == "d") {
					...
					if ($recs) {
						...
						for ($k = 0; $k < $rec_num; $k++) {
							...
							$tmp = array("id" => "", "da" => "", "de" => "", "am" => "", "ty" => "", "fa" => "","up" => "");
							$tmp["ty"] = "pd";
							$tmp["id"] = "";
							$tmp["da"] = "";
							$tmp["de"] = $dea;
							...
							for ($l = 0; $l < $deb_num; $l++) {
								...
								if ($l == 0 && $recs[$k]["slsdtl_taxable"] == "t" && $recs[$k]["pickdtl_cost"] * $recs[$k]["pickdtl_qty"] != 0) {
									$tmp["am"] = number_format($recs[$k]["pickdtl_cost"] * $recs[$k]["pickdtl_qty"] * $out_arr[$j][2]["pick_taxrate"] / 100, 2, ".", ",");
									$tmp["up"] = "Tax";
								} else {
									$tmp["am"] = "";
									$tmp["up"] = "";
				...
				$tmp = array("id" => "", "da" => "", "de" => "", "am" => "", "fx" => "", "fa" => "","up"=>"", "ty" => "");
				$tmp["ty"] = "c";
				...
				if ($details == "d") {
					...
					if ($recs) {
						...
						for ($k = 0; $k < $rec_num; $k++) {
							...
							$deb_num = count($deb_arr);
							for ($l = 0; $l < $deb_num; $l++) {
								$tmp = array("id" => "", "da" => "", "de" => "", "am" => "", "tx" => "", "fa" => "","up" => "","ty" => "");
								$tmp["ty"] = "cd";
								$tmp["id"] = "";
								$tmp["da"] = "";
								$tmp["de"] = $deb_arr[$l];
								$tmp["up"] = "";
								$am = $recs[$k]["cmemodtl_cost"] * $recs[$k]["cmemodtl_qty"] * -1 * $out_arr[$j][2]["cmemo_taxrate"] / 100;
								if ($l == 0 && $recs[$k]["cmemodtl_taxable"] == "t") {
									$tmp["am"] = number_format($am, 2, ".", ",");
								} else {
									$tmp["am"] = "0.00";
								}
								array_push($dtl_arr, $tmp);
				...
				if ($out_arr[$j][2]["rcpt_type"] == "ca")
					$description = "Cash";
				else if ($out_arr[$j][2]["rcpt_type"] == "ch")
					$description = "Check #" . $out_arr[$j][2]["rcpt_check_no"];
				else if ($out_arr[$j][2]["rcpt_type"] == "cc")
					$description = "Credit Card";
				else if ($out_arr[$j][2]["rcpt_type"] == "dc")
					$description = "Discount Apply";
				else if ($out_arr[$j][2]["rcpt_type"] == "bc")
					$description = "Bounced Check";
				else if ($out_arr[$j][2]["rcpt_type"] == "ot")
