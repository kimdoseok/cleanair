...
class Styles extends IC {
	...
	function getStylesPO($code) {
		$this->query = "SELECT * FROM styles WHERE styl_po_no = '$code' ";
	...
	function deleteStyles($code) {
		$this->query = "DELETE FROM styles WHERE styl_code = '$code' ";
	...
	function getStylesList($condition="", $filtertext="", $reverse="f", $page=1, $limit=20) {
		...
		if (empty($condition)) $this->query = "SELECT * FROM styles ORDER BY styl_code ";
		else $this->query = "SELECT * FROM styles WHERE styl_po_no ='$condition' ORDER BY styl_code ";
