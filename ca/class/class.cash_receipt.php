	...
	function getRcptsList($condition="", $filtertext="", $reverse="f", $page=1, $limit=20) {
		...
		if (empty($filtertext) || !isset($filtertext)) {
			if ($condition == "code") {
				...
				$this->query = "SELECT * FROM rcpts ORDER BY rcpt_desc  ";
			} else {
				$this->query = "SELECT * FROM rcpts ORDER BY rcpt_id ";
			}
		} else {
			if ($condition == "code") {
				$this->query = "SELECT * FROM rcpts WHERE rcpt_id  LIKE '$filtertext%' ORDER BY rcpt_id ";
			} else if ($condition == "desc") {
				$this->query = "SELECT * FROM rcpts WHERE rcpt_desc  LIKE '$filtertext%' ORDER BY rcpt_desc  ";
			} else {
				$this->query = "SELECT * FROM rcpts ORDER BY rcpt_id ";
			}
		}
