...
?>
				  <table width="100%" border="0" cellspacing="1" cellpadding="0">
                    <tr align="right"> 
                      <td colspan="8"><strong>List Users</strong></td>
                    </tr>
                    <tr>
...
<?php
	$limit = 100;
	$c = new Users();
	$numrows = $c->getUsersRows($cn, $ft);
	$recs = $c->getUsersList($cn, $ft, $rv, $page, $limit);
	$n = new Navigates();
...
?>
                            <td width="10%" align="center"> 
                              <a href="<?= htmlentities($_SERVER['PHP_SELF'])."?ty=v&user_code=".urlencode($recs[$i]["user_code"]) ?>"><?= $recs[$i]["user_code"] ?></a>
                            </td>
                            <td width="25%"> 
                              <?= $recs[$i][user_name] ?>
                            </td>
                            <td width="64%"><?= $recs[$i][user_desc] ?></td>
                            <td width="1%"><?= ($recs[$i][user_active] != "f")?"X":"&nbsp;" ?></td>
                          </tr>
<?php
	}
	if ($numrecs == 0) {
