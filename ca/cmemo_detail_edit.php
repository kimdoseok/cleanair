<?php
	...
	if ($ht == "e") $slsdtl = $_SESSION[$cmemodtl_edit];
	else $slsdtl = $_SESSION[$cmemodtl_add];
	...
	if (!empty($cmemodtl_item_code)) {
		...
		if ($tarr = $t->getItems($cmemodtl_item_code)) {
			...
			if ((empty($cmemodtl_msrp) || $cmemodtl_msrp==0) && empty($cmemodtl_cost)) $cmemodtl_cost = $tarr["item_msrp"];
			$not_found_item = 0;
		} else {
			$not_found_item = 1;
...
<!--
	...
	function EditDtl() {
		...
		f.cmd.value = "cmemo_detail_sess_edit";
		f.method = "post";
		f.submit();
	}
...
//-->
						...
						<INPUT TYPE="hidden" name="cmemo_id" value="<?= $cmemo_id ?>">
						<INPUT TYPE="hidden" name="cmemodtl_cmemo_id" value="<?= $cmemodtl_cmemo_id ?>">
						<?= $f->fillHidden("ht", $ht) ?>
						<?= $f->fillHidden("ty", $ty) ?>
						<?= $f->fillHidden("did", $did) ?>
                    <tr align="right"> 
                      <td colspan="8"><strong>Edit Credit Memo Detail</strong></td>
                    </tr>
                    <tr align="right"> 
                      <td colspan="8" align="left"><font size="2">
							   | <a href="<?php echo htmlentities($_SERVER['PHP_SELF'])."?ty=a&ht=<?= $ht ?>&cmemo_id=<?= $cmemo_id ?>" ?>"><?= $label[$lang]["New_1"] ?></a> |
...
?>
                <tr> 
                  ...
                  <td width="308"> 
                    <?= $f->fillTextBox("cmemodtl_item_desc", stripslashes($cmemodtl_item_desc), 32, 250, "inbox") ?>
                  </td>
                </tr>
                <tr> 
                ...
                <tr> 
                  ...
                  <td> 
                    <INPUT TYPE="checkbox" NAME="cmemodtl_taxable" VALUE="t" <?= ($cmemodtl_taxable == "t")?"checked":"" ?>>
                  </td>
                </tr>
              </table> </td>
          </tr>
                    ...
                    <tr align="right"> 
                      <td colspan="8"> <table width="100%" border="0" cellspacing="1" cellpadding="0">
                          <tr> 
                            ...
                            <th bgcolor="gray" width="10%"><font color="white"><?= $label[$lang]["PO_no"] ?></font></th>
