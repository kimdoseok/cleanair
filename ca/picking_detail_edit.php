<?php
	...
	$amount = $pickdtl_cost * $pickdtl_qty;
...
<!--
	...
	function EditDtl() {
		...
		f.cmd.value = "pick_detail_sess_edit";
		f.method = "post";
		f.submit();
	}
	...
	function openPickBrowse(objname)  {
		var f = document.forms[0];
		var url = 'picking_slsdtls_popup.php';
...
//-->
						...
						<INPUT TYPE="hidden" name="pickdtl_id" value="<?= $pickdtl_id ?>">
						<INPUT TYPE="hidden" name="slsdtl_pick_id" value="<?= $slsdtl_pick_id ?>">
						<?= $f->fillHidden("ht", $ht) ?>						
						<?= $f->fillHidden("ty", $ty) ?>						
						<?= $f->fillHidden("did", $did) ?>
                    <tr align="right"> 
                      <td colspan="8"><strong>Edit Picking Detail</strong></td>
                    </tr>
                    <tr align="right"> 
                      <td colspan="8" align="left"><font size="2">
							   | <a href="<?php echo htmlentities($_SERVER['PHP_SELF'])."?ty=a&ht=$ht" ?>"><?= $label[$lang]["New_1"] ?></a> |
          ...
          <tr bgcolor="white"> 
            <td width="477" align="right"><table width="100%" border="0" cellspacing="1" cellpadding="0">
				<tr> 
				  ...
				  <td width="308"> 
					<?= $f->fillTextBox("pickdtl_code", $pickdtl_code, 20, 32, "inbox") ?>
